 ...
 *                    URDF: axis="1 0 0" (x-axis) with rpy="0 0 1.5708"
 *                    实际效果：绕 Y 轴旋转，控制腿在 X-Z 平面的侧向摆动
 ...
 * 
 * 坐标系说明：
 * - HAA 关节：虽然 URDF 中定义为 axis="1 0 0"，但由于 visual/collision 的
 *   rpy="0 0 1.5708" 旋转，实际上关节在父坐标系中绕 Y 轴旋转
 * - 旋转方向：正角度使 X 轴向 Z 轴旋转（右手定则，Y 轴向上）
 * 
...
public:
     ...
     * @param haa 髋关节侧摆角度 [rad] (绕 Y 轴旋转)
     * @param hfe 髋关节俯仰角度 [rad]
     * @param kfe 膝关节角度 [rad]
     * @return Eigen::Vector3d 足端在基座坐标系中的位置
     * 
     * 注意：HAA 角度是绕 Y 轴的旋转，正角度使 X 轴向 Z 轴旋转
     */

...
bool CrossingStateMachine::checkBodyForwardShiftComplete(const RobotState& state) const {
    ...
    bool diagonal_positive_extended =
        state.sliding_positions[0] >= physical_sliding_max_(0) - compactMargin(physical_sliding_min_(0), physical_sliding_max_(0)) &&
        state.sliding_positions[2] >= physical_sliding_max_(2) - compactMargin(physical_sliding_min_(2), physical_sliding_max_(2));

    bool diagonal_negative_extended =
        state.sliding_positions[1] <= physical_sliding_min_(1) + compactMargin(physical_sliding_min_(1), physical_sliding_max_(1)) &&
        state.sliding_positions[3] <= physical_sliding_min_(3) + compactMargin(physical_sliding_min_(3), physical_sliding_max_(3));

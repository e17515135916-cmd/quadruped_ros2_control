#!/usr/bin/env python3
"""
Dog2 + CHAMP + Gazebo Fortress 主启动文件

这是主启动文件，用于启动完整的 Dog2 + CHAMP + Gazebo Fortress 系统。
...
Requirements: 2.1, 2.2, 2.3, 2.4, 2.5, 6.1, 6.2, 6.3, 6.4, 6.5, 6.6
"""
...
def generate_launch_description():
    ...
    # 处理 URDF - 写入临时文件以便 Gazebo 加载完整 URDF（包含 ros2_control）
    import tempfile
    ...
    # 将 URDF 写入临时文件
    urdf_temp_file = tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.urdf')
    urdf_temp_file.write(robot_description_config.toxml())
    ...
    # 使用 -file 参数从临时文件加载完整 URDF（包含 ros2_control 标签）

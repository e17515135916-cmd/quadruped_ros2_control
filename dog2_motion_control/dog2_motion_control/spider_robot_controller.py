...
class SpiderRobotController(Node):
    ...
    def __init__(self, config_path: str = None):
        ...
        self.target_velocity = (0.0, 0.0, 0.0)   # 目标速度（用于平滑停止）
    ...
    def start(self):
        ...
        try:
            ...
            for leg_id in ['lf', 'rf', 'lh', 'rh']:
                ...
                initial_positions[joint_names['rail']] = initial_rail
                initial_positions[joint_names['haa']] = initial_haa
                initial_positions[joint_names['hfe']] = initial_hfe
                initial_positions[joint_names['kfe']] = initial_kfe

            # 发送一次位置命令（作为启动姿态）
            self.joint_controller.send_joint_commands(initial_positions)
            self.get_logger().info('Initial standing pose commanded on startup')
        except Exception as e:
            self.get_logger().error(f'Failed to publish initial standing pose: {e}')
        ...
        for leg_id in ['lf', 'rf', 'lh', 'rh']:
            ...
            if ik_result is None:
                ...
                if self.last_valid_joint_positions[leg_id] is not None:
                    ...
                    # 如果没有上一个有效配置，使用零位
                    self.get_logger().error(
                        f"No last valid config for {leg_id}, using zero position"
                    )
                    s_m, haa_rad, hfe_rad, kfe_rad = 0.0, 0.0, 0.0, 0.0
            else:
                # IK成功：保存为有效配置

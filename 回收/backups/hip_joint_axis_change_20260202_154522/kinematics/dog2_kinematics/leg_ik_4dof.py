...
class LegIK4DOF:
    ...
    def solve(self, foot_position_base: np.ndarray, 
        ...
        haa_distance_yz = np.sqrt(haa_to_foot[1]**2 + haa_to_foot[2]**2)
        ...
        # HAA 角度（从 Y 轴到足端的角度）
        solution.haa = np.arctan2(haa_to_foot[2], haa_to_foot[1])
        
        # 步骤 5: 计算髋关节俯仰轴（HFE）在基座坐标系中的位置
        # HFE 相对于 HAA 有一个偏移
        hfe_offset_rotated = np.array([
            self.geometry.haa_to_hfe_offset[0],
            self.geometry.haa_to_hfe_offset[1] * np.cos(solution.haa) - 
            self.geometry.haa_to_hfe_offset[2] * np.sin(solution.haa),
            self.geometry.haa_to_hfe_offset[1] * np.sin(solution.haa) + 
        ...
        # 计算在 HFE 平面内的坐标
        plane_x = hfe_to_foot[0]
        plane_y = (hfe_to_foot[1] * np.cos(solution.haa) + 
    ...
    def forward_kinematics(self, prismatic: float, haa: float,
        ...
        hfe_offset_rotated = np.array([
            self.geometry.haa_to_hfe_offset[0],
            self.geometry.haa_to_hfe_offset[1] * np.cos(haa) - 
            self.geometry.haa_to_hfe_offset[2] * np.sin(haa),
            self.geometry.haa_to_hfe_offset[1] * np.sin(haa) + 
    ...
    def _solve_2r_plane_ik(self, x: float, y: float, l1: float, l2: float) -> Tuple[bool, float, float]:
        ...
        # 膝关节角度（肘向下配置，负角度）
        kfe = -np.arccos(cos_kfe)
        
        # 求解髋关节俯仰角度
        alpha = np.arctan2(y, x)
        beta = np.arccos((l1**2 + distance_sq - l2**2) / (2.0 * l1 * distance))

...
IKSolution LegIK4DOF::solve(const Eigen::Vector3d& foot_position_base,
    ...
    double haa_distance_yz = std::sqrt(haa_to_foot.y() * haa_to_foot.y() + 
                                       haa_to_foot.z() * haa_to_foot.z());
    
    if (haa_distance_yz < 1e-6) {
        solution.error_msg = "足端位置太接近 HAA 轴";
        return solution;
    }
    
    // HAA 角度（从 Y 轴到足端的角度）
    solution.haa = std::atan2(haa_to_foot.z(), haa_to_foot.y());
    
    // 步骤 5: 计算髋关节俯仰轴（HFE）在基座坐标系中的位置
    // HFE 相对于 HAA 有一个偏移
    Eigen::Vector3d hfe_offset_rotated;
    hfe_offset_rotated.x() = geometry_.haa_to_hfe_offset.x();
    hfe_offset_rotated.y() = geometry_.haa_to_hfe_offset.y() * std::cos(solution.haa) - 
                             geometry_.haa_to_hfe_offset.z() * std::sin(solution.haa);
    hfe_offset_rotated.z() = geometry_.haa_to_hfe_offset.y() * std::sin(solution.haa) + 
    ...
    // 计算在 HFE 平面内的坐标
    // X 方向：沿着腿的前后方向
    // Y 方向：沿着腿的上下方向（在 HAA 旋转后的坐标系中）
    double plane_x = hfe_to_foot.x();
    double plane_y = hfe_to_foot.y() * std::cos(solution.haa) + 
...
Eigen::Vector3d LegIK4DOF::forwardKinematics(double prismatic, double haa,
    ...
    Eigen::Vector3d hfe_offset_rotated;
    hfe_offset_rotated.x() = geometry_.haa_to_hfe_offset.x();
    hfe_offset_rotated.y() = geometry_.haa_to_hfe_offset.y() * std::cos(haa) - 
                             geometry_.haa_to_hfe_offset.z() * std::sin(haa);
    hfe_offset_rotated.z() = geometry_.haa_to_hfe_offset.y() * std::sin(haa) + 
    ...
    foot_pos.x() = hfe_pos.x() + plane_x;
    foot_pos.y() = hfe_pos.y() + plane_y * std::cos(haa);
    foot_pos.z() = hfe_pos.z() + plane_y * std::sin(haa);

...
ros2 run robot_state_publisher robot_state_publisher --ros-args -p robot_description:="$(cat $TEMP_URDF)" &
RSP_PID=$!

# 等待一下让robot_state_publisher启动
sleep 2

# 发布joint_states (所有关节设为0)
echo "正在发布joint_states..."
ros2 run joint_state_publisher joint_state_publisher --ros-args -p robot_description:="$(cat $TEMP_URDF)" &
JSP_PID=$!
